 ...
 */
export const EDUCATION_HIERARCHY: EducationLevelType[] = [
...
export const educationHelpers = {
  ...
  getEducationLevelRank: (degree: string): number => {
    ...
    const exactMatch = EDUCATION_HIERARCHY.findIndex(level => 
      degree.toLowerCase() === level.toLowerCase()
    );
   ...
   */
  detectOverlappingEducation: (
    educationList: SelectEducation[], 
    excludeId?: string
  ): Array<{ 
    ...
    for (let i = 0; i < relevantEducation.length; i++) {
      for (let j = i + 1; j < relevantEducation.length; j++) {
        ...
        const overlap = educationHelpers.checkDateOverlap(
          edu1Start, edu1End, edu2Start, edu2End
        );
  ...
  calculateTotalEducationYears: (educationList: SelectEducation[]): number => {
    ...
    for (const edu of educationList) {
      const duration = educationHelpers.calculateDuration(
  ...
  generateSortOrder: (existingEducation: SelectEducation[]): number => {
    if (existingEducation.length === 0) return 1;
...
export const educationCommonQueries = {
  ...
  getEducationSummary: (profileId: string) => ({
    ...
    select: {
      ...
      hasOngoing: sql<boolean>`BOOL_OR(${education.isOngoing})`,
