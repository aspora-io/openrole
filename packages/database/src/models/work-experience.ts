...
export const workExperienceHelpers = {
   ...
   */
  validateAchievements: (achievements: string[]): { valid: boolean; errors: string[] } => {
    const errors: string[] = [];
   ...
   */
  calculateDurationMonths: (startDate: Date, endDate: Date | null = null): number => {
   ...
   */
  calculateDurationYears: (startDate: Date, endDate: Date | null = null): number => {
   ...
   */
  formatDuration: (startDate: Date, endDate: Date | null = null): string => {
    const months = workExperienceHelpers.calculateDurationMonths(startDate, endDate);
  ...
  calculateTotalExperience: (experiences: SelectWorkExperience[]): number => {
    const totalMonths = experiences.reduce((total, exp) => {
      return total + workExperienceHelpers.calculateDurationMonths(
   ...
   */
  sortExperiences: (
    experiences: SelectWorkExperience[], 
    sortBy: 'date' | 'sortOrder' = 'sortOrder'
   ...
   */
  withDurationCalculations: (
    experiences: SelectWorkExperience[]
  ): WorkExperienceWithDuration[] => {
  ...
  getNextSortOrder: (existingExperiences: SelectWorkExperience[]): number => {
    if (existingExperiences.length === 0) return 1;
  ...
  extractAllSkills: (experiences: SelectWorkExperience[]): string[] => {
    ...
    const uniqueSkills = Array.from(
      new Set(allSkills.map(skill => skill.trim().toLowerCase()))
    );
    
    // Return with original casing
    return uniqueSkills.map(uniqueSkill => {
      const original = allSkills.find(skill => 
        skill.trim().toLowerCase() === uniqueSkill
      );
