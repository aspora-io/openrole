...
export const portfolioItems = pgTable('portfolio_items', {
    ...
    return {
        ...
        typeValid: check('portfolio_items_type_valid', sql `${table.type} IN ('project', 'article', 'design', 'code', 'document', 'link')`),
        fileSizePositive: check('portfolio_items_file_size_positive', sql `${table.fileSize} > 0`),
        validationStatusValid: check('portfolio_items_validation_status_valid', sql `${table.validationStatus} IN ('pending', 'valid', 'invalid', 'unreachable')`),
...
export const portfolioHelpers = {
    ...
    needsValidation: (item) => {
        ...
        return (!item.lastValidationCheck ||
     ...
     */
    filterAndSort: (items, filters = {}, sort = { field: 'sortOrder', direction: 'asc' }) => {
        let filtered = items;
        // Apply filters
        ...
        if (filters.technologies && filters.technologies.length > 0) {
            filtered = filtered.filter(item => item.technologies &&
                filters.technologies.some(tech => item.technologies.some(itemTech => itemTech.toLowerCase().includes(tech.toLowerCase()))));
        }
        if (filters.hasFile !== undefined) {
            filtered = filtered.filter(item => filters.hasFile ? !!item.filePath : !item.filePath);
        }
        if (filters.hasUrl !== undefined) {
            filtered = filtered.filter(item => filters.hasUrl ? !!item.externalUrl : !item.externalUrl);
        }
        ...
        if (filters.validationStatus && filters.validationStatus.length > 0) {
            filtered = filtered.filter(item => filters.validationStatus.includes(item.validationStatus));
        }
        // Apply sorting
        filtered.sort((a, b) => {
            let aValue;
     ...
     */
    getNextSortOrder: (existingItems) => {
     ...
     */
    reorderItems: (items, targetId, newSortOrder) => {
        const updates = [];
...
export const urlValidationHelpers = {
     ...
     */
    createValidationQueueEntry: (item) => {
        if (!item.externalUrl)
            return null;
        return {
...
export const technologyHelpers = {
     ...
     */
    getTechnologySuggestions: (existingTech, allPortfolioTech) => {
