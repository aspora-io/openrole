 ...
 * @date 2025-09-30
 */
import { pgTable, uuid, varchar, text, integer, boolean, date, timestamp, jsonb, index, unique, check } from 'drizzle-orm/pg-core';
...
export const candidateProfileHelpers = {
     ...
     */
    calculateVerifiedBadge: (profile) => {
     ...
     */
    getDisplayName: (profile, userFirstName, userLastName) => {
     ...
     */
    getMaskedContactInfo: (profile, userEmail) => {
        const masked = {};
        if (profile.contactInfoVisible) {
            if (userEmail)
                masked.email = userEmail;
            if (profile.phoneNumber)
                masked.phone = profile.phoneNumber;
        }
        else {
            // Show masked versions
            if (userEmail)
                masked.email = userEmail.replace(/(.{2}).+(@.+)/, '$1***$2');
            if (profile.phoneNumber)
                masked.phone = profile.phoneNumber.replace(/(.{3}).+(.{2})/, '$1***$2');
        }
...
export const candidateProfileQueries = {
     ...
     */
    getSelectFieldsByPrivacy: (privacyLevel) => {
